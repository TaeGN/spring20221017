			...
			<div class="col">
				...
				<div>
					<c:forEach items="${board.fileName }" var="name">
						<div>
							<img class="img-fluid img-thumbnail" alt="" src="${imgUrl }/${board.id }/${URLEncoder.encode(name, 'utf-8')}">
						</div>
					</c:forEach>
				</div>
		...
		for(const item of list) {
			...
			const replyDiv = `
				<div class="list-group-item d-flex">
					...
					<div>
						<button class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modifyReplyFormModal" data-reply-id="\${item.id}" id="\${modifyReplyButtonId}"><i class="fa-solid fa-pen"></i></button>
						<button class="btn btn-light" data-bs-toggle="modal" data-bs-target="#removeReplyConfirmModal" data-reply-id="\${item.id}" id="\${removeReplyButtonId}"><i class="fa-solid fa-eraser"></i></button>
					</div>
				</div>`;
