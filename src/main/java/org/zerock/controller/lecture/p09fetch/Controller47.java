...
public class Controller47 {
	...
	public Map<String, Object> method9(@PathVariable int id) {
		...
		if(supplier != null) {
			map.put("messageDiv", "공급자 정보가 조회 완료되었습니다.");
			map.put("supplier", supplier);
		} else {
			map.put("messageDiv", "공급자 정보를 조회하지 못하였습니다.");
		}
	...
	public Map<String, Object> method11(@RequestBody JavaBean19 supplier) {
		...
		if(cnt == 1) {
			map.put("message", supplier.getId() + "번 공급자 정보 변경 성공");
		} else {
			map.put("message", supplier.getId() + "번 공급자 정보 변경 실패");
		}
	...
	@PostMapping("sub13")
	@ResponseBody
	public Map<String, Object> method13(@RequestBody JavaBean18 customer) {
		Map<String, Object> map = new HashMap<>();
		int cnt = mapper.insertCustomer(customer);
		if(cnt == 1) {
			map.put("message", customer.getId() + "번 고객 정보 추가 성공");
		} else {
			map.put("message", "고객 정보 추가 실패");
		}
		return map;
	}
	...
	public Map<String, Object> method15(@PathVariable int id) {
		...
		if(cnt == 1) {
			map.put("message", id + "번 공급자 정보 삭제 성공");
		} else {
			map.put("message", id + "번 공급자 정보 삭제 실패");
		}
